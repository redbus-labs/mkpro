...
public class CompactCommand implements Command {
    ...
    public void execute(String[] args, MkProContext context) throws Exception {
        FilterConfig fc = context != null ? context.getFilterConfig() : null;
        if (fc == null) {
            try {
                fc = CentralMemory.getInstance().getFilterConfig();
            } catch (Exception e) {
                fc = new FilterConfig();
            }
            if (context != null) {
    ...
    private void saveFilterConfig(FilterConfig fc) {
        try {
            ...
            CentralMemory.getInstance().saveFilterConfig(fc);
        } catch (Exception ignored) {}
    }

    private void rebuildRunnerWithConfig(MkProContext context, FilterConfig fc) {
        if (context != null && context.getAgentManager() != null && context.getAgentConfigs() != null) {
