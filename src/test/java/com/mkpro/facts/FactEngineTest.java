...
public class FactEngineTest {
    ...
    void verifySphereVolume() {
        ...
        assertFalse(result.containsKey("error"), "Evaluation returned error: " + result.get("error"));
        assertTrue(result.containsKey("result"), "Result should contain computed value");
        double value = ((Number) result.get("result")).doubleValue();
    ...
    @Test
    void queryKeywordReturnsMatchingFactsAndTriples() {
        engine.addFact(new Fact("SECURITY", "AUTH", "Always sanitize input strings", 0.9));
        engine.getRelationshipGraph().addTriple(new RelationshipTriple("SECURITY", "enforces", "sanitize_input", "test", 0.9));

        String queryResult = engine.query("sanitize");
        assertNotNull(queryResult);
        assertTrue(queryResult.contains("Always sanitize input strings"));
        assertTrue(queryResult.contains("SECURITY enforces sanitize_input"));

        // Blank or null keyword
        assertEquals("", engine.query(null));
    ...
    @Test
    void statsNotEmpty() {
        FactEngine.FactStats stats = engine.getStats();
        assertNotNull(stats);
        assertTrue(stats.formulas() > 0 || stats.relationships() > 0);
    }
}