...
public class RelationshipGraph {
    ...
    private final List<RelationshipTriple> triples = new CopyOnWriteArrayList<>();
    ...
    public void addTriple(RelationshipTriple triple, double confidence) {
        ...
        outgoing.computeIfAbsent(subject, k -> new CopyOnWriteArrayList<>())
                .add(new Edge(predicate, object, domain, confidence));
        incoming.computeIfAbsent(object, k -> new CopyOnWriteArrayList<>())
    ...
    /**
     * Find relationship triples matching optional subject, predicate, and/or object filters.
     * Pass null for any argument to match all.
     */
    public List<RelationshipTriple> findTriples(String subject, String predicate, String object) {
        return this.triples.stream()
                .filter(t -> subject == null || (t.subject() != null && t.subject().equalsIgnoreCase(subject)))
                .filter(t -> predicate == null || (t.predicate() != null && t.predicate().equalsIgnoreCase(predicate)))
                .filter(t -> object == null || (t.object() != null && t.object().equalsIgnoreCase(object)))
                .toList();
    }

    public List<RelationshipTriple> getAllTriples() {
        return new ArrayList<>(this.triples);
    }
